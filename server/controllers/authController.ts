...
async function saveRefreshToken(user: any, refreshToken: any) {
	...
	console.log("saveRefreshToken tokenData", tokenData, refreshToken);
	if (tokenData) {
...
export const loginUser = async (req: Request, res: Response) => {
	...
	console.log("loginUser req user", req.user);
...
export const refreshUser = async (req: Request, res: Response) => {
	...
	if (!userData || !refreshTokenDB) {
		console.log("!!userData || !refreshTokenDB", userData, refreshTokenDB);
...
export const updateUserInfo = async (req: Request, res: Response) => {
	try {
        ...
        console.log('updateUserInfo',{ name, lastName, age });
		...
		if (user) {
            ...
            const returnUser = await User.findById((userData as any).id).select('-password');
...
export const uploadAvatar = async (req: Request, res: Response) => {
	try {
		...
		const avatarString = `data:image/png;base64,${req.file?.buffer.toString('base64')}`;
